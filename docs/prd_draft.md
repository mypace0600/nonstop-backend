...
##### 정책 동의 프로세스
- **회원가입 시 동의**: `POST /api/v1/auth/signup` 요청 시 `agreedPolicyIds` (동의한 정책 ID 목록)를 포함하여 전송합니다. 회원가입 시 필수 정책에 동의하지 않으면 가입이 차단됩니다.
- **로그인**: 로그인은 이미 가입 시 정책에 동의한 사용자를 대상으로 하므로 별도의 정책 검증을 수행하지 않습니다.
- **추가 동의**: 로그인된 상태에서 선택적 정책(마케팅 등)에 동의하거나 업데이트할 경우 `POST /api/v1/policies/agree` API를 사용합니다.
