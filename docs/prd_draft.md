...
## 9. MVP 정책 보완 사항 (v2.5.19 신규)

PM/기획 관점에서 검토한 결과, MVP 출시 전 반드시 정의되어야 할 정책들입니다.

### 9.1 Google OAuth 만 14세 체크 정책 🔴 HIGH

#### 문제점
- 이메일 가입: 회원가입 시 `birthDate` 필수 → 만 14세 미만 가입 차단 가능
- Google OAuth: 가입과 로그인이 동시 발생 → 계정 생성 후에야 생년월일 수집
- **법적 이슈**: 만 14세 미만 아동의 개인정보 수집은 법정대리인 동의 필요

#### 해결 방안

**Option A: OAuth 첫 로그인 시 생년월일 선수집 (권장)**
```
Google 로그인 → 신규 사용자 감지 → 생년월일 입력 화면 (계정 생성 전)
                                   ↓
                        만 14세 이상 확인 → 계정 생성 → 정책 동의 → 서비스 이용
                                   ↓
                        만 14세 미만 → "만 14세 이상만 가입 가능합니다" 안내 → 가입 차단
```

**API 변경 필요:**
- `POST /api/v1/auth/google` 응답에 `isNewUser: boolean` 추가
- 신규 사용자인 경우 클라이언트에서 생년월일 입력 후 `POST /api/v1/auth/google/complete` 호출
- 또는 기존 API에서 `birthDate` 파라미터를 받아 신규 사용자 생성 시 검증

**Option B: 계정 생성 후 차단 (차선책)**
```
Google 로그인 → 계정 생성 (birthDate=null) → hasBirthDate=false 응답
                                            ↓
                              생년월일 입력 → 만 14세 미만 → 계정 즉시 삭제 + 로그아웃
```
- 단점: 계정이 잠깐이라도 생성됨, UX가 어색함

#### 결정 사항
- [ ] Option A / Option B 선택 필요
- [ ] API 명세 확정 필요

---

### 9.2 인증 코드 보안 강화 정책 🔴 HIGH

#### 문제점
- 6자리 숫자 인증 코드 (100만 가지 조합)
- 발송 Rate Limit: 1분당 1회 (OK)
- **확인 시도 횟수 제한 없음** → 브루트포스 공격 가능
...
| 최대 시도 횟수 | 5회 |
| 초과 시 | 인증 코드 즉시 무효화 |
| Redis Key | `verification:attempt:{email}` (TTL: 5분) |
| 응답 | `400 Bad Request` (Code: `MAX_ATTEMPTS_EXCEEDED`) |

**구현 위치:**
- 회원가입 이메일 인증: `POST /api/v1/auth/email/verify`
- 학교 웹메일 인증: `POST /api/v1/verification/email/confirm`
...
| 옵션 | 설명 | 장점 | 단점 |
|------|------|------|------|
| A. 유지 (익명화) | 작성자를 "알 수 없음"으로 표시 | 커뮤니티 콘텐츠 보존 | 탈퇴해도 글이 남음 |
| B. 삭제 | 모든 게시글/댓글 soft delete | 깔끔한 탈퇴 | 대화 맥락 손실 |
| C. 선택권 | 탈퇴 시 사용자가 선택 | 유연성 | 구현 복잡 |

**권장**: Option A (유지 + 익명화)
- 게시글/댓글의 `user_id`는 유지하되, 조회 시 닉네임을 "알 수 없음"으로 표시
- `isMine` 필드는 항상 `false`로 처리 (수정/삭제 불가)

**9.3.2 채팅 처리**
| 유형 | 처리 방안 |
|------|----------|
| 1:1 채팅 | 상대방에게 채팅방 유지, 탈퇴 사용자는 "알 수 없음"으로 표시 |
| 그룹 채팅 | 자동 퇴장 처리, 시스템 메시지 "OOO님이 탈퇴하였습니다" |
| 메시지 내역 | 유지 (발신자만 익명화) |
...
| 대기 기간 | 없음 (즉시 가능) |
| 데이터 복구 | 불가 |
| 이전 닉네임 사용 | 가능 (다른 사용자가 사용 중이면 불가) |

---

### 9.4 채팅 차단 처리 정책 🟡 MEDIUM

#### 1:1 채팅에서의 차단
| 상황 | 처리 |
|------|------|
| A가 B를 차단 | A→B 메시지 전송 가능, B→A 메시지 전송 시 403 |
| 차단 후 채팅방 | 채팅방 유지, A는 계속 접근 가능 |
| 과거 메시지 | 양쪽 모두 기존 메시지 열람 가능 |
| 차단 해제 후 | 모든 기능 복구 |

#### 그룹 채팅에서의 차단
| 상황 | 처리 |
|------|------|
| A가 B를 차단 (같은 그룹) | 그룹 채팅은 정상 작동 |
| B의 메시지 | A에게도 정상 표시 (그룹은 예외) |
| 이유 | 그룹 대화 맥락 유지 필요 |

> **Note**: 그룹 채팅에서는 차단이 메시지 숨김에 영향을 주지 않습니다.
> 차단은 1:1 채팅 생성과 1:1 메시지 전송에만 영향을 줍니다.

---

### 9.5 익명 게시판 접근 정책 🔴 HIGH

#### 문제점
- 미인증 사용자(`is_verified=false`)가 익명 게시판에 글을 쓸 수 있으면 악용 가능
- 신고해도 실질적 추적/제재 어려움

#### 정책
| 게시판 타입 | 미인증 사용자 | 인증 사용자 |
|------------|--------------|------------|
| GENERAL | 읽기 ✅ / 쓰기 ✅ | 읽기 ✅ / 쓰기 ✅ |
| NOTICE | 읽기 ✅ / 쓰기 ❌ | 읽기 ✅ / 쓰기 ❌ (Admin만) |
| QNA | 읽기 ✅ / 쓰기 ✅ | 읽기 ✅ / 쓰기 ✅ |
| **ANONYMOUS** | 읽기 ✅ / 쓰기 ❌ | 읽기 ✅ / 쓰기 ✅ |

**에러 응답 (미인증 + 익명 게시판 작성 시도):**
```json
{
  "success": false,
  "data": { "universityRequired": true },
  "message": "익명 게시판은 대학 인증 후 이용 가능합니다."
}
```

---

### 9.6 대학 인증 상태 관리 정책 🟡 MEDIUM

#### 9.6.1 웹메일 인증 시 대학 불일치 처리
```
현재 university_id = A대학 (프로필에서 선택)
웹메일 인증 시도 = B대학 도메인
```

**정책:**
- 웹메일 도메인에서 추출한 대학으로 `university_id` 덮어쓰기
- 단, 인증 전 확인 메시지 표시 (클라이언트)
  - "입력하신 이메일은 B대학교 소속입니다. B대학교로 인증하시겠습니까?"

#### 9.6.2 학생증 인증 상태 관리
| 상태 | 설명 | 재신청 가능 |
|------|------|------------|
| PENDING | 심사 대기 중 | ❌ (기존 요청 취소 후 가능) |
| ACCEPTED | 승인 완료 | - |
| REJECTED | 반려됨 | ✅ (즉시 재신청 가능) |

**중복 요청 방지:**
- 한 사용자당 PENDING 상태 요청은 1개만 허용
- 에러: `400 Bad Request` (Code: `VERIFICATION_ALREADY_PENDING`)

#### 9.6.3 인증 취소/해제 시 콘텐츠 처리
- 대학 커뮤니티 접근 차단
- 기존 작성 게시글/댓글은 유지 (수정 가능, 새 글 작성 불가)
- 공개 시간표 자동 비공개 전환

---

### 9.7 신고 처리 및 통보 정책 🟡 MEDIUM

#### 신고 처리 결과 통보
| 처리 결과 | 신고자 통보 | 피신고자 통보 |
|----------|------------|--------------|
| BLIND | ✅ "신고가 처리되었습니다" | ✅ "회원님의 게시글이 커뮤니티 가이드라인 위반으로 블라인드 처리되었습니다" |
| DELETE | ✅ "신고가 처리되었습니다" | ✅ "회원님의 게시글이 삭제되었습니다" |
| REJECT | ✅ "검토 결과 위반 사항이 없습니다" | ❌ 통보 없음 |
...
| 5회 | 7일 글쓰기 제한 |
| 10회 | 30일 글쓰기 제한 |
| 15회 | 영구 글쓰기 제한 (Admin 검토 후 해제) |

> **Note**: 누적 제재는 MVP 이후 구현 예정
...
#### "읽음"의 정의
- 채팅방 진입 시 자동으로 `PATCH /api/v1/chat/rooms/{roomId}/read` 호출
- 해당 시점까지의 모든 메시지를 읽음 처리
- 푸시 알림 클릭만으로는 읽음 처리 안 됨 (채팅방 진입 필요)

#### 읽음 표시 (상대방에게)
- **MVP**: 읽음 표시 미노출 (카카오톡의 '1' 미표시)
- **향후**: 읽음 시간 표시 옵션 추가 검토

---

### 9.9 시간표 공개 범위 정책 🟢 LOW

#### 현재 정책
- 같은 대학 + 인증 사용자 → 모든 공개 시간표 조회 가능
...
> **Note**: MVP에서는 현재 정책 유지, 사용자 피드백 후 옵션 추가 검토

---

### 9.10 콘텐츠 모더레이션 정책 🟡 MEDIUM

#### 닉네임 필터링
- 금칙어 목록 기반 필터링
- 욕설, 혐오표현, 성적 표현 포함 시 거부
- 에러: `400 Bad Request` (Code: `INAPPROPRIATE_NICKNAME`)

#### 프로필 이미지
- MVP: 별도 필터링 없음 (신고 기반 사후 처리)
- 향후: AI 기반 이미지 모더레이션 검토
...
| 기능 | PRD 섹션 | 상태 | 비고 |
|------|----------|------|------|
| 비밀번호 재설정 | 3.1.9 | ❌ 미구현 | 임시 비밀번호 발급, `password_must_change` 플래그 |
| 인증 코드 시도 횟수 제한 | 9.2 | ❌ 미구현 | 브루트포스 방지 |
| Google OAuth 만 14세 체크 | 9.1 | ❌ 미구현 | 법적 필수 |
| 익명 게시판 인증 제한 | 9.5 | ❌ 미구현 | 악용 방지 |

### 10.2 권장 구현 (P1) 🟡

| 기능 | PRD 섹션 | 상태 | 비고 |
|------|----------|------|------|
| 게시판 관리 API | 3.4.3 | ❌ 미구현 | Admin CRUD |
| 탈퇴 시 콘텐츠 익명화 | 9.3 | ⚠️ 검증 필요 | 정책대로 구현되었는지 확인 |
| 신고 처리 결과 알림 | 9.7 | ❌ 미구현 | 신고자/피신고자 통보 |
| 학생증 중복 요청 방지 | 9.6.2 | ⚠️ 검증 필요 | PENDING 중복 체크 |

### 10.3 선택 구현 (P2) 🟢

| 기능 | PRD 섹션 | 상태 | 비고 |
|------|----------|------|------|
| 조회수 중복 방지 | 3.5 | ❌ 미구현 | Redis/Cookie 기반 |
| 닉네임 금칙어 필터 | 9.10 | ❌ 미구현 | 콘텐츠 모더레이션 |
| 누적 제재 자동화 | 9.7 | ❌ 미구현 | MVP 이후 |
